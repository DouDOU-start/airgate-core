...
func TestCalculate_AccountRateSnapshotIndependent(t *testing.T) {
	// 渠道成本倍率仅作快照：不影响 actual/billed，落 account_rate_multiplier
	// 供查询期现算渠道成本（total × 快照）。
	c := NewCalculator()
	res := c.Calculate(CalculateInput{
		InputCost:   1.0,
	...
	if !almostEqual(res.AccountRateMultiplier, 1.5) {
		t.Fatalf("AccountRateMultiplier = %v, want 1.5", res.AccountRateMultiplier)
	}
	// 现算口径：渠道成本 = total × 快照
	if got := res.TotalCost * res.AccountRateMultiplier; !almostEqual(got, 3.0) {
		t.Fatalf("derived account cost = %v, want 3.0", got)
	}
}
...
func TestCalculate_ZeroAccountRate_DefaultsToOne(t *testing.T) {
	...
	if !almostEqual(res.AccountRateMultiplier, 1.0) {
		t.Fatalf("AccountRateMultiplier = %v, want 1.0 (account_rate defaults to 1.0)", res.AccountRateMultiplier)
	}
}
...
func TestCalculate_CacheCreationInTotal(t *testing.T) {
	// 缓存写费用（含 5m/1h 双档合计）与其他分项一起进 total。
	c := NewCalculator()
	res := c.Calculate(CalculateInput{
		InputCost:         0.2,
		OutputCost:        0.3,
	...
	if !almostEqual(res.CacheCreationCost, 0.4) {
		t.Fatalf("CacheCreationCost = %v, want 0.4", res.CacheCreationCost)
	}
}
