...
// asset storage，原地替换为 /assets-runtime/... 形式的 publicURL，并返回本次新落盘的
// object_key 列表，方便任务删除时同步回收。
//
...
//
// 所有落盘文件统一使用 purpose=task-input，object_key 由 core 内部按
// task-input/<uid>/<yyyymm>/<id>.<ext> 规则生成。pluginID 不进入 storage key
// （插件归属信息通过 task 表的 plugin_id 字段维护，存储层不感知调用方）。
func normalizeTaskInputAssets(ctx context.Context, storage *AssetStorage, userID int64, input map[string]any) ([]string, error) {
	if storage == nil || len(input) == 0 || userID <= 0 {
		return nil, nil
	}
	var objectKeys []string
	if err := walkAndNormalize(ctx, storage, userID, input, &objectKeys); err != nil {
		return nil, err
	}
	return objectKeys, nil
}

func walkAndNormalize(ctx context.Context, storage *AssetStorage, userID int64, node any, objectKeys *[]string) error {
	switch v := node.(type) {
	case map[string]any:
		for k, child := range v {
			if s, ok := child.(string); ok {
				replaced, objectKey, err := maybeStoreDataURI(ctx, storage, userID, s)
				if err != nil {
			...
			if err := walkAndNormalize(ctx, storage, userID, child, objectKeys); err != nil {
				return err
			}
		}
	case []any:
...
// 解码后通过 storage.Store 落盘，返回新的 publicURL 与 object_key；否则原样返回。
func maybeStoreDataURI(ctx context.Context, storage *AssetStorage, userID int64, ref string) (string, string, error) {
	...
	asset, err := storage.Store(ctx, userID, AssetPurposeTaskInput, mime, extensionForContentType(mime), data)
	if err != nil {
		return "", "", fmt.Errorf("store input asset: %w", err)
	}
	return asset.PublicURL, asset.ObjectKey, nil
}
