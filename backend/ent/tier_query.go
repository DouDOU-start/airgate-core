...
func (tq *TierQuery) QueryUsers() *UserQuery {
...
func (tq *TierQuery) FirstID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (tq *TierQuery) FirstIDX(ctx context.Context) int {
...
func (tq *TierQuery) OnlyID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (tq *TierQuery) OnlyIDX(ctx context.Context) int {
...
func (tq *TierQuery) IDs(ctx context.Context) (ids []int, err error) {
...
func (tq *TierQuery) IDsX(ctx context.Context) []int {
	ids, err := tq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (tq *TierQuery) WithUsers(opts ...func(*UserQuery)) *TierQuery {
	query := (&UserClient{config: tq.config}).Query()
...
func (tq *TierQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tier, error) {
	...
	if query := tq.withUsers; query != nil {
		if err := tq.loadUsers(ctx, query, nodes,
			func(n *Tier) { n.Edges.Users = []*User{} },
			func(n *Tier, e *User) { n.Edges.Users = append(n.Edges.Users, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TierQuery) loadUsers(ctx context.Context, query *UserQuery, nodes []*Tier, init func(*Tier), assign func(*Tier, *User)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Tier)
	for i := range nodes {
	...
	query.Where(predicate.User(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(tier.UsersColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.tier_users
		if fk == nil {
			return fmt.Errorf(`foreign-key "tier_users" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "tier_users" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (tq *TierQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(tier.Table, tier.Columns, sqlgraph.NewFieldSpec(tier.FieldID, field.TypeInt))
	_spec.From = tq.sql
