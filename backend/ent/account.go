...
type Account struct {
	...
	// AES-GCM 加密的凭证 JSON（map[string]string）
	...
	// 账号成本倍率：账号成本 = total × rate_multiplier
	RateMultiplier float64 `json:"rate_multiplier,omitempty"`
	// 进入当前状态的原因（给运维看）
	ErrorMsg string `json:"error_msg,omitempty"`
	// 已废弃：账号池走渠道；账号侧恒为 false
...
func (*Account) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case account.FieldID, account.FieldPriority, account.FieldWeight, account.FieldMaxConcurrency:
			values[i] = new(sql.NullInt64)
		case account.FieldName, account.FieldPlatform, account.FieldType, account.FieldCredentialsEnc, account.FieldEmail, account.FieldState, account.FieldErrorMsg:
...
func (a *Account) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case account.FieldCredentialsEnc:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field credentials_enc", values[i])
			} else if value.Valid {
				a.CredentialsEnc = value.String
			}
...
func (a *Account) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("credentials_enc=<sensitive>")
	builder.WriteString(", ")
