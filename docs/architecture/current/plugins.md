...
- **health**:健康监控;`prober.go` 探测 + `aggregator.go` 聚合;分组元信息/可见性过滤经 `groups.list`,自有 DB 连接仅读写 `group_health_probes`;Core 的 `/status` 反代至此(目标插件由 config `plugins.status_plugin` 指定,默认 `airgate-health`)。
