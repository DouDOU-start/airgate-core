...
`internal/plugin/host_service.go`。插件经单一 `CoreInvokeService.Invoke/InvokeStream` 通道,按 method 字符串调用。**当前 19 个 method**(host_service.go 顶部 const 块):
...
| 元数据 | `groups.list`(可选 `{public_only, user_id}` 过滤状态页可见分组,返回含 `note`/`status_visible`)、`platforms.list`、`models.list` |
| 用户 | `users.get`、`users.update_balance`(add/subtract + 流水,`idempotency_key` 必填防重复入账;epay 充值经此入账) |
...
- **scheduling model 映射**(`plugin/scheduling_model.go`):三级查询——模型目录 `Metadata["scheduling_model"]`(精确 ID) → 插件路由声明 `Metadata["scheduling_model_map"]`(前缀映射表,openai 插件已声明 claude-\*→GPT,env 覆盖由插件读取) → 历史硬编码兜底(未声明插件兼容,勿扩展)。
