...
- `internal/relay/pricing` — 价目表缓存 + token→cost 纯函数。图像：`quality:size` 表命中 > 裸 size > quality 空/auto 时按 medium→high→low 回退已知 size > per_request > token；表未命中且无 token 时按表内默认档（medium:1024x1024）按张兜底，避免逆向渠道生图 $0。
