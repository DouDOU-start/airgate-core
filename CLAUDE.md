...
异步任务分树（internal/relay/task，与 pipeline 并列的独立子系统）：
  ...
  提交：缺价预检（按次价或 pricing_extra.video.per_second×时长估价）→ 余额预扣（同步扣
  user.balance）→ failover≤3（同 outcome 判定）→ task 表落库；查询读本地快照不穿透上游；
...
- **转发路由（/v1、/v1beta、/suno）错误一律按入口协议的原生错误形态**（`internal/relay/errfmt` 按 EntryProtocol 分发：openai/anthropic/gemini），不用 `response.*`；上游错误解析语义后按入口协议重建（状态码保留上游原值），不做字节透传；管理面照旧 `response.*`。
